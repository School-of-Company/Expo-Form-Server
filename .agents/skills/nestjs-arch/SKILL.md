---
name: nestjs-arch
description: NestJS project architecture and conventions for this team — module structure and file naming, interface-token dependency injection, @nestjs/config for settings, Zod-derived DTO classes for every controller and service argument, ZodValidationPipe for request shape, store pattern, logging, thin controllers, consumer error handling, and constructor-injected unit tests. Use when creating or modifying NestJS modules, services, controllers, DTOs, providers, or their tests.
---
...
- **DTOs** — every argument crossing a controller or service boundary is a DTO **class** built with
  `createZodDto()` (not an interface, not loose positional params); responses are DTOs too. The Zod
  schema is the source of truth — derive internal types with `z.infer`.
- **Validation** — request shape via the global `ZodValidationPipe`; business rules throw from the
  service. Schemas built at runtime from a stored field spec are validated with `safeParse` in the
  service, since the pipe only covers statically declared DTOs.
- **Controllers** — bind and delegate, nothing else. Do **not** `import type` a DTO: it's a class and
  the pipe needs its runtime metadata.
...
| Zod DTOs, `ZodValidationPipe`, `@nestjs/config`  | `.agents/skills/nestjs-arch/references/dto-validation.md`      | Adding an endpoint or config value     |
